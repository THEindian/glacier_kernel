...
/*#define GLACIER_SDMC_CD_N_TO_SYS PM8058_GPIO_PM_TO_SYS(GLACIER_GPIO_SDMC_CD_N)*/
...
static unsigned int glacier_sdslot_status(struct device *dev)
{
	...
	status = (unsigned int) gpio_get_value(GLACIER_SDMC_CD_N_TO_SYS);
...
static struct mmc_platform_data glacier_sdslot_data = {
	...
	.status_irq	= MSM_GPIO_TO_INT(GLACIER_SDMC_CD_N_TO_SYS),
...
int __init glacier_init_mmc(unsigned int sys_rev)
{
	...
	*/
	...
	set_irq_wake(MSM_GPIO_TO_INT(GLACIER_SDMC_CD_N_TO_SYS), 1);

	msm_add_sdcc(4, &glacier_sdslot_data,
			MSM_GPIO_TO_INT(GLACIER_SDMC_CD_N_TO_SYS),
